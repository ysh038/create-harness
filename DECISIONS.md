...
## 38. v1.0.0 — `/qa` 블라인드 QA 에이전트 + 안정성 약속 + 안전한 업그레이드

### 배경

코드를 짠 에이전트가 테스트까지 쓰면 같은 착각을 공유한다 — 구현에 맞춰 쓴 테스트는 항상 통과한다.
처음 아이디어는 "코드를 모르는 에이전트가 앱을 써 보고, 그 흐름을 그대로 테스트로 만든다"였지만,
관찰한 흐름을 테스트로 만들면 **현재 동작(버그 포함)을 녹화**하는 것이라 같은 함정에 빠진다.

### 결정 1 — `/qa`: 테스트 생성기가 아니라 버그 발견기

- `qa-tester` 서브에이전트가 목적 하나를 받아 실제 사용자처럼 앱을 쓰고, **리포트**를 낸다
  (막힘 / 헷갈림 / 반응 부족 / 접근성 신호 / 기술 로그 + 테스트 후보).
- 메인 에이전트가 코드를 보고 분류(버그 / UX 개선 / 의도된 동작 / 도구 한계)한 뒤,
  **확인된 버그만** 테스트로 옮긴다. 버그가 있는 상태에서 쓰므로 테스트는 처음부터 실패한다 — 녹화 문제가 사라진다.
- 메인 에이전트는 QA 에이전트에게 목적·주소·사용자 유형·테스트 계정만 넘긴다. 코드 힌트 금지.
- 사용자 유형(persona): 처음온사용자 / 급한사용자 / 키보드사용자 / 모바일사용자.
- `/ux-review`(코드 보고 시각 품질)와 역할 분리: `/qa`는 화면만 보고 쓰는 흐름.

**"코드를 모름"의 보장 수준이 도구마다 다르다:**
- Claude Code: `tools: mcp__playwright` 로 브라우저 도구만 허용, `omitClaudeMd: true` 로 AGENTS.md/CLAUDE.md도
  안 읽음 (안내 문서에 폴더 구조·화면 목록이 있어서 중요). Playwright MCP는 에이전트 전용으로 인라인 정의.
- Cursor: 서브에이전트가 부모 도구를 전부 물려받고 도구 단위 제한이 없다. `readonly: true` + 지시문으로만 요구.
  Cursor도 `.claude/agents/` 를 읽으므로 둘 다 선택 시 파일 하나를 공유한다 (이름 충돌 방지).
- 에이전트 이름은 `qa-tester` — 스킬 `/qa` 와 같은 이름이면 헷갈린다.

### 실험 (구현 전)

이 환경에 Claude Code CLI가 없어 에이전트 설정 파일 자체(도구 제한·omitClaudeMd·인라인 MCP)는 실행하지 못했다
(공식 문서로 확인). 대신 가장 불확실한 부분 — **지시문만으로 쓸 만한 리포트가 나오는가** — 를 실험했다.
문제 5개를 심은 테스트 앱(로그인 → 글쓰기)에 브라우저 도구만 쓰는 서브에이전트를 두 번 돌렸다.

- 1차 (처음온사용자): 심은 문제 5개 전부 발견 + 심지 않은 실제 문제 1개(로그인 실패 시 이메일도 지워짐).
  **오탐 1개**: 도구 호출 지연을 앱이 느린 것으로 착각해 "로딩 표시 필요"를 테스트 후보에까지 넣음.
  → 지시문에 "속도 판단 금지, 동작 후 텍스트로 상태 재확인" 추가.
- 2차 (키보드사용자, 수정된 지시문): 속도 오판 사라짐. 새 문제 2개:
  (a) 버튼 포커스 후 Enter가 안 먹힘 → 직접 확인하니 **실험용 브라우저 도구의 한계** (클릭은 정상).
  (b) 1차 실행 때 쌓인 네트워크 로그를 이번 실행 것으로 착각.
  → "조작이 안 먹으면 한 번만 클릭으로 대조, 되면 도구 한계로 표시" + "시작 이후 로그만" 추가.
- 교훈: QA 에이전트 리포트는 **분류 없이 믿으면 안 된다.** 그래서 워크플로에 메인 에이전트의 분류 단계를
  필수로 두고, 속도·도구 한계 항목을 특히 의심하라고 적었다.

### 결정 2 — 1.0.0 = 안정성 약속

기능 크기가 아니라 "무엇을 함부로 안 바꾸는가"로 1.0을 정의했다. README "Stability (1.0+)":
- 공개(바꾸면 major): CLI 플래그·`--config` 키, `config.json` 키, `design-references.json` 항목 형태,
  설치 위치, 워크플로 이름, **무엇이 차단되는가**, 업그레이드 동작.
- **새 차단 규칙은 major에서만.** minor에서 새 검사는 경고로 들어온다 — 사용자의 작업 흐름을 갑자기 막지 않기 위해.
- 내부(minor에서도 바뀜): 검사 스크립트 내부, 메시지 문구, 경고 판정 기준과 기본값, 워크플로 문장.

### 결정 3 — 안전한 업그레이드

기존에는 재실행 시 내용이 다른 파일이 **전부** `.harness/incoming/` 으로 갔다. 검사 스크립트도 예외가 아니라,
0.7.0의 권한 수정 같은 중요한 변경이 업그레이드해도 실제로는 적용되지 않았다.

- `manifest.json` 의 해시와 현재 파일이 같으면 = 사용자가 손대지 않은 하네스 파일 → 새 버전으로 교체 (`↻`)
- 다르면 → `incoming` (기존과 동일). 설치 기록이 없는 파일(하네스 이전부터 있던 파일)은 절대 교체하지 않음.
- `incoming` 에 둔 파일은 manifest에서 **이전 해시를 유지** — 지우면 다음 업그레이드 때 판단 근거가 사라진다.

### 함께 고친 것

- **재실행 시 하네스가 설치한 `tokens.css` 를 기존 위반으로 탐지** — 토큰 정의 파일이라 원시 색상값이 정상인데,
  재실행하면 baseline 에 올리고 30-design-system 규칙 문장까지 바뀌었다. 업그레이드 = 재실행이라 1.0에서 중요해짐. 탐지에서 제외.
- 오래된 표현: `shell-gate.mjs`(없는 이름), "Write/StrReplace 도구를 쓰세요"(Edit가 빠짐), "Cursor의 preToolUse 훅으로"(Claude도 씀).
- `test/pre-write-gate.test.ts` 의 `echo '...' |` 입력 전달을 stdin(`input`)으로 통일.
- 저장소 AGENTS.md에 테스트 작성 규칙 추가 (지금까지 기존 파일을 보고 짐작해야 했음).

### 남은 한계

- Claude Code에서 `qa-tester` 설정(도구 제한·인라인 Playwright MCP)을 실제로 실행해 보지 못했다 — 첫 실사용에서 확인 필요.
- `config.json` 은 `disclaimerAcceptedAt` 타임스탬프 때문에 재실행마다 내용이 달라진다. 손대지 않았으면 교체되고
  손댔으면 incoming 으로 가므로 해는 없지만 소음이다.

### 변경 파일

- `templates/core/agents/qa-tester.md`: 신규 — QA 에이전트 정의
- `templates/core/workflows/qa.md`: 신규 — `/qa` 워크플로
- `src/registry.ts`: `qa` 워크플로, `buildAgentActions` (Claude/Cursor 분기)
- `src/manifest.ts`, `src/types.ts`, `src/cli.ts`: 안전한 업그레이드 (`readManifest`, `updated`)
- `src/detect.ts`: tokens.css 탐지 제외
- `templates/core/AGENTS.md`, `CLAUDE.md`: `/qa` 안내
- `templates/core/gates/*`: 오래된 표현 정리
- `README.md`: Upgrading, Stability (1.0+), What you get
- `AGENTS.md`(저장소): 테스트 작성 규칙
- `test/scaffold.test.ts`: QA 4 + 업그레이드 4 + detect 1, `test/pre-write-gate.test.ts`: stdin 통일
